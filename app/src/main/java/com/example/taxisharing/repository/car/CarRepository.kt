...
class CarRepository {
    ...
    suspend fun getCardDataFromFile(fileName: String, context: Context): CarResponseModel {

        try {

        val res = FileUtil().readTextFromAsset(fileName, context)
        val resObj = Gson().fromJson(res, CarResponseModel::class.java)
        return resObj

        }catch (e: Exception) {
            return CarResponseModel(data = null)
        }
    }
